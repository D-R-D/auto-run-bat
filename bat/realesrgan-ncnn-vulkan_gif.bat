...
if exist "video" (
	...
	for /F "delims=" %%B in ('dir /b video') do (
		...
		"realesrgan-ncnn-vulkan.exe" -i temp\video_tmp -o temp\frame_tmp -n realesr-animevideov3 -s 2
		...
		for /F "delims=" %%D in ('ffmpeg\bin\ffprobe.exe -i video\%%B -v 0 -of csv^="p=0" -select_streams V:0 -show_entries stream^=avg_frame_rate 2^>^&1') do (
			for /F "delims=" %%G in ('PowerShell -command "[Math]::Round(%%D, 2,[MidpointRounding]::AwayFromZero);"') do (
				echo %%G
				"ffmpeg\bin\ffmpeg.exe" -r %%G -i "temp\frame_tmp\frame%%08d.png" -r %%G -pix_fmt yuv420p "video_enc\%%B"
			)
		)
